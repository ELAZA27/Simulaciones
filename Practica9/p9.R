...
mmax <- max(p$m)
mmin <- min(p$m)
p$m <- ((p$m - mmin)*(p$m - mmin) / (mmax - mmin))+1 # masas de 0 a 1
...
for (iter in 1:tmax) {
  ...
  graphics.off()
  png(paste("histx_", tl, ".png", sep = ""))
  hist(p$m, main=paste("Paso", iter), freq=TRUE, xlab = "Velocidad normalizada")
  graphics.off()
  png(paste("histy_", tl, ".png", sep = ""))
  hist(p$y, main=paste("Paso", iter), freq=TRUE, xlab = "Velocidad normalizada")
  graphics.off()
  png(paste("relacion_t", tl, ".png", sep=""))
  pairs(p[,1:4], labels = c("Velocidad X", "Velocidad Y", "Carga", "Masa"), col = c("red"))
  graphics.off()
