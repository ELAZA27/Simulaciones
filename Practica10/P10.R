...
for(init in c(20, 50, 80)){
  ...
  for(replica in 1:5){
    ...
    for (iter in 1:tmax) {
      ...
      padres <- parSapply(cluster,1:rep,function(x){return(sample(1:tam, 2, replace=FALSE))}) #selecciÃ³n de padres
    ...
    endS=Sys.time()
    mejorS=mejor
    datos=rbind(datos,c(init=init,Secuencial=difftime(endS,startS,units='secs'),Paralelo=difftime(endP,startP,units='secs'),Paral=(optimo - mejorP) / optimo,Secu=(optimo - mejorS) / optimo))
  }
  
}
stopCluster(cluster)

names(datos) = c("init","Secuencial","Paralelo","Paral","Secu")
datos1 = datos[,1:2]
